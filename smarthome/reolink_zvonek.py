...
def build_case():
    ...
    inner_body = inner_body.moveTo(between_holes/2, 0).circle(screw_hole / 2).moveTo(-between_holes/2, 0).circle(screw_hole / 2).cutThruAll()
    ...
    outer_body = outer_body.faces('<Z').workplane().moveTo(between_holes/2, 0).circle(
        calculate_pla_bore_diameter(screw_hole) / 2).moveTo(-between_holes/2, 0).circle(
